...
SQLite3.prototype.createIndex = function (model, field, cb) {
    this.command('CREATE INDEX ' + this.escapeName(model+'-'+field) + 
        ' on ' + this.tableEscaped(model) + '(' + this.escapeName(field) + ')', cb);
};

SQLite3.prototype.dropIndex = function (model, field, cb) {
    this.command('DROP INDEX ' + this.escapeName(model+'-'+field), cb);
};
