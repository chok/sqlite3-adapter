...
SQLite3.prototype.autoupdate = function (cb) {
    ...
    Object.keys(this._models).forEach(function (model) {
        ...
        self.queryAll('PRAGMA table_info(' + self.tableEscaped(model)+')', function (err, fields) {
            self.queryAll('PRAGMA INDEX_LIST(' + self.tableEscaped(model) + ');', function(indexErr, indexes) {
                if (err || fields.length === 0) {
                    self.createTable(model, done);
                } else {
                    self.alterTable(model, fields, done, indexes);
                }
            });
        });
    });
...
SQLite3.prototype.createIndex = function (model, fields, cb) {
    var name = this.escapeName(model+'-'+fields.join('-'));
    fields = fields.map(this.escapeName);

    this.command('CREATE INDEX ' + name + 
        ' on ' + this.tableEscaped(model) + '(' + fields.join(', ') + ')', cb);
};

SQLite3.prototype.dropIndex = function (model, fields, cb) {
    var name = this.escapeName(model+'-'+fields.join('-'));
    this.command('DROP INDEX ' + name, cb);
};

SQLite3.prototype.createTable = function (model, cb, createIndex) {
    createIndex = createIndex || true;
    var self = this;
    var props = this._models[model].properties;
    var propsKeys = Object.keys(props);

    var indexes = this._models[model].settings.indexes;
    indexes = indexes || {};
    ...
    this.command('CREATE TABLE ' + this.tableEscaped(model) +
        ' (\n  ' + this.propertiesSQL(model) + '\n)', function() {
            ...
            (function ci() {
                var key = propsKeys.shift();
                var property = props[key];
                if (typeof key === 'undefined') {
                    return cis();
                }

                if (typeof property.index !== 'undefined' && property.index) {
                    self.createIndex(model, [key], ci);
                }
            })();
            ...
            function cis() {
                ...
                if (typeof index.properties !== 'undefined') {
                    self.createIndex(model, index.properties, cis);
                }
            }
    });
};

SQLite3.prototype.alterTable = function (model, actualFields, done, actualIndexes) {
    var self = this;
    ...
    var indexes = { create: [], remove: []};

    var multiIndexes = m.settings.indexes;
    multiIndexes = multiIndexes || {};
    multiIndexes = Object.keys(multiIndexes).map(function(name) {
        return multiIndexes[name];
    });

    multiIndexes.forEach(function (index) {
        var name = model+'-'+index.properties.join('-');
        var found = false;
        ...
        if (!found) {
            indexes.create.push(index.properties);
        }
    });

    propNames.forEach(function (propName) {
        var property = m.properties[propName];
        var hasIndex = typeof property.index !== 'undefined' && property.index === true;
        ...
        actualIndexes.forEach(function (i) {
            ...
            if (name === propName) {
                indexFound = i;
                if (!hasIndex) {
                    indexes.remove.push(propName);
                }
            }
        });

        if (typeof indexFound === 'undefined' && hasIndex) {
            indexes.create.push(propName);
        }
    ...
    (function addIndexes() {
        if (indexes.create.length > 0) {
            (function ci(err) {
                var index = indexes.create.shift();
                if (typeof index === 'undefined') {
                ...
                self.createIndex(model, index, ci);
            })();
        } else {
            removeIndexes();
        }
    ...
    function removeIndexes() {
        if (indexes.remove.length > 0) {
            (function cr() {
                var index = indexes.remove.shift();
                if (typeof index === 'undefined') {
                    return rebuild();
                }
                if (typeof index === 'string') {
