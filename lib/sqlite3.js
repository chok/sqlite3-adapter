...
SQLite3.prototype.createIndex = function (model, field, cb) {
    var sql = 'CREATE INDEX ' + this.escapeName(model+'-'+field) + 
        ' on ' + this.tableEscaped(model) + '(' + this.escapeName(field) + ')';

    console.log(sql);

    this.command(sql, cb?cb:function(){});
};

SQLite3.prototype.dropIndex = function (model, field, cb) {
    var sql = 'DROP INDEX ' + this.escapeName(model+'-'+field);
